...
function MullerLyer() {
  ...
  return (
    <GameShell cite="Müller-Lyer, 1889" instructions={
        <ol>
          <li>Two horizontal lines appear — the top line is the reference.</li>
          <li>Drag the <strong>slider</strong> to adjust the bottom line's length.</li>
          <li>Make the bottom line look the same length as the top one.</li>
          <li>Click "lock answer" when satisfied — your perceptual bias is revealed.</li>
        </ol>
      } phase={phase} headline="measure what you see" explain="Arrowheads pull perceived length away from physical length." onBegin={begin} onReset={begin} footer={`adjusted length ${v}px`} results={[{ label: "true top", value: "180 px" }, { label: "your match", value: `${v} px` }, { label: "bias", value: `${v - 180} px` }]} doneText="The illusion is the gap between the slider's number and the line your eyes believed." >
...
function Ebbinghaus() {
  ...
  return (
    <GameShell cite="Ebbinghaus, 1902" instructions={
        <ol>
          <li>Two blue circles appear, each surrounded by rings of different sizes.</li>
          <li>The <strong>left circle is the target</strong> — its size is fixed.</li>
          <li>Drag the slider to resize the right circle until it <em>looks</em> the same size.</li>
          <li>Click "lock answer" to see how far context shifted your perception.</li>
        </ol>
      } phase={phase} headline="context has gravity" explain="Identical centres can feel different when their neighbours change scale." onBegin={begin} onReset={begin} footer={`match diameter ${v}px`} results={[{ label: "target", value: "52 px" }, { label: "match", value: `${v} px` }, { label: "bias", value: `${v - 52} px` }]} doneText="Surrounding circles shift the felt size of the centre, even when you know the trick.">
...
function ChangeBlindness() {
  ...
  return (
    <GameShell cite="Rensink et al., 1997" instructions={
        <ol>
          <li>A scene flickers with brief white flashes between each view.</li>
          <li>One object changes colour across the flicker.</li>
          <li><strong>Click the changing object</strong> as soon as you spot it.</li>
          <li>The flash masks the change — this is what makes it hard to see.</li>
        </ol>
      } phase={phase} headline="what vanished?" explain="A blank blink interrupts the scene, and one ordinary object changes." onBegin={begin} onReset={begin} footer={found ? "change found" : "flickering scene"} results={[{ label: "found", value: found ? "yes" : "no" }, { label: "time", value: `${Math.round(rt)} ms` }, { label: "change", value: "clock" }]} doneText="Change blindness appears when attention has not indexed the changing object.">
