...
function Flanker() {
  ...
  return (
    <GameShell cite="Eriksen & Eriksen, 1974" instructions={<><span className="kbd">←</span><span className="kbd">→</span> answer the centre arrow</>} phase={phase} headline="ignore the chorus" explain="The middle arrow is the only one that counts. The flanks will try to vote." onBegin={begin} onReset={begin} footer={`trial ${Math.min(trial + 1, 24)}/24`} results={[{ label: "congruent", value: `${mean(con)} ms` }, { label: "incongruent", value: `${mean(inc)} ms` }, { label: "cost", value: `${Math.max(0, mean(inc) - mean(con))} ms` }]} doneText="A flanker cost means irrelevant arrows still entered the response system.">
...
function VisualSearch() {
  ...
  return (
    <GameShell cite="Treisman & Gelade, 1980" instructions="click the red square as the set size grows" phase={phase} headline="find the conjunction" explain="A target defined by colour and shape asks for serial attention." onBegin={begin} onReset={begin} footer={`trial ${Math.min(trial + 1, 9)}/9 · set size ${items.length || 12}`} results={[{ label: "mean search", value: `${mean(rows.map((r) => r.rt))} ms` }, { label: "small sets", value: `${mean(rows.filter((r) => r.size === 12).map((r) => r.rt))} ms` }, { label: "large sets", value: `${mean(rows.filter((r) => r.size === 32).map((r) => r.rt))} ms` }]} doneText="Conjunction search usually slows as more distractors join the display.">
