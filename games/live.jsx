...
function MOT() {
  ...
  return <GameShell cite="Pylyshyn & Storm, 1988" instructions="watch the pastel blue targets, then click the four original targets" phase={phase} headline="keep the tags" explain="Four dots start highlighted, then all dots become identical and move." onBegin={begin} onReset={begin} footer={select ? `select targets · ${picks.length}/4` : "tracking interval"} results={[{ label: "correct", value: `${correct}/4` }, { label: "objects", value: 8 }, { label: "tracked", value: 4 }]} doneText="MOT asks attention to preserve object identities across motion and distraction."><canvas ref={canvasRef} width="640" height="380" onClick={click} style={{ maxWidth: "100%", border: "1px solid #b8d8ef", background: "#f3fbff" }}></canvas></GameShell>;
}
...
function BeatTap() {
  ...
  return <GameShell cite="Repp, 2005" instructions="tap the stage with the 100 BPM clicks" phase={phase} headline="meet the beat" explain="A metronome clicks every 600 ms. Tap along until the run ends." onBegin={begin} onReset={begin} footer={`${taps.length}/18 taps`} results={[{ label: "mean ITI", value: `${mean(iti)} ms` }, { label: "ITI SD", value: `${sd(iti)} ms` }, { label: "target", value: "600 ms" }]} doneText="Synchronization is prediction with feedback: your taps reveal how tightly the next beat was anticipated."><div className="center-stack" onClick={tap}><div className="mono-big">♪</div><button className="choice-btn">tap here</button></div></GameShell>;
}
