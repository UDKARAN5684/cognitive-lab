...
function Trolley() {
  ...
  return <GameShell cite="Foot, 1967" instructions={
        <ol>
          <li>Four ethical dilemmas are presented one at a time.</li>
          <li>Each asks whether a harmful act is justified to save more lives.</li>
          <li>Answer <strong>Yes</strong> or <strong>No</strong> based on your moral intuition.</li>
          <li>There are no right answers — your pattern across scenarios is the result.</li>
        </ol>
      } phase={phase} headline="moral fingerprint" explain="Four scenarios vary action, contact, intention, and tradeoff." onBegin={begin} onReset={begin} footer={`scenario ${i + 1}/4`} results={[{ label: "yes", value: yes }, { label: "no", value: 4 - yes }, { label: "pattern", value: yes >= 3 ? "util" : "mixed" }]} doneText="Trolley answers are less a verdict than a contour: where action and harm start to feel different."><div className="center-stack"><p className="task-explain">{qs[i]}</p><div className="choice-row"><button className="choice-btn" onClick={() => pick(true)}>yes</button><button className="choice-btn" onClick={() => pick(false)}>no</button></div></div></GameShell>;
}
...
function EmotionRecog() {
  ...
  return <GameShell cite="Ekman & Friesen, 1976" instructions={
        <ol>
          <li>A stylised face appears showing one of six expressions.</li>
          <li>Identify the emotion from the options shown below the face.</li>
          <li><strong>Click the matching label</strong> — joy, anger, fear, sadness, surprise, or disgust.</li>
          <li>6 faces in total.</li>
        </ol>
      } phase={phase} headline="read the face" explain="Stylized faces stand in for the classic expression photographs." onBegin={begin} onReset={begin} footer={`face ${i + 1}/6`} results={[{ label: "score", value: `${score}/6` }, { label: "faces", value: 6 }, { label: "format", value: "SVG" }]} doneText="Emotion labels are fast guesses from features, context, and learned categories."><div className="center-stack"><Face emotion={items[i] || "joy"} /><div className="choice-row">{emotions.map((e) => <button className="choice-btn" key={e} onClick={() => pick(e)}>{e}</button>)}</div></div></GameShell>;
}
...
function Marshmallow() {
  ...
  return <GameShell cite="Mischel, 1972" instructions={
        <ol>
          <li>You can take <strong>$1 immediately</strong>, or wait 60 seconds to receive <strong>$2</strong>.</li>
          <li>Click "wait for $2" to start the countdown.</li>
          <li>You may take $1 at any point during the wait by clicking "take $1 now".</li>
          <li>One trial — real patience, not a hypothetical choice.</li>
        </ol>
      } phase={phase} headline="one now, two later" explain="An adult-sized waiting room for delay of gratification." onBegin={begin} onReset={begin} footer={waiting ? `${left}s left` : "choice point"} results={[{ label: "choice", value: choiceMade }, { label: "waited", value: `${waited}s` }, { label: "reward", value: choiceMade === "waited" ? "$2" : "$1" }]} doneText="The famous finding became more complicated over time, but the lived task remains a study in strategy and temptation."><div className="center-stack">{waiting ? <><div className="progress"><i style={{ width: `${(60 - left) / 60 * 100}%` }}></i></div><button className="choice-btn" onClick={take}>take $1 now</button></> : <div className="choice-row"><button className="choice-btn" onClick={take}>take $1 now</button><button className="primary-btn" onClick={wait}>wait for $2</button></div>}</div></GameShell>;
}
